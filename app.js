...
const { routeCallingcode } = require('./routes/callingcode.js');
...
const { routeZodiac } = require('./routes/zodiac.js');
...
const { routeHashids } = require('./routes/hashids.js');
const { routeOrdinal } = require('./routes/ordinal.js');
...
const { routeSunrise } = require('./routes/sunrise.js'); // sunrise
...
const { routeMarkdown: routeMarkdown2 } = require('./routes/md2html.js');
...
http.createServer(async (req, res) => {
  ...
  try {
    ...
    if (u.pathname === '/ordinal') {
      try { return routeOrdinal(u, res, json); }
      catch (e) { return json(res, 400, { error: e.message }); }
    }
    if (u.pathname === '/hashids') {
      try { return routeHashids(u, res, json); }
      catch (e) { return json(res, 400, { error: e.message }); }
    }
    ...
    if (u.pathname === '/currency') {
      try { return routeCurrency(u, res, json); }
      catch (e) { return json(res, 400, { error: e.message }); }
    }
    ...
    if (u.pathname === '/whois') {
            ...
            if (u.pathname === '/netmask') {
              try { return routeNetmask(u, res, json); }
              catch (e) { return json(res, 400, { error: e.message }); }
            }
            if (u.pathname === '/hmac') {
  ...
  if (u.pathname === '/duration') return routeDuration(u, res, json, reqBody);
    ...
    if (u.pathname === '/luhn') {
            ...
            if (u.pathname === '/vat') {
      try { return routeVat(u, res, json); }
    ...
    if (u.pathname === '/iban') {
            ...
            if (u.pathname === '/isbn') {
              try { return routeIsbn(u, res, json); }
              catch (e) { return json(res, 500, { error: e.message }); }
            }
            if (u.pathname === '/imei') {
  ...
  if (u.pathname === '/base32') return routeBase32(u, res, json, reqBody);
  if (u.pathname === '/json2sql') return routeJson2Sql(u, res, json, reqBody);
      ...
      try { return routeCronexpr(u, res, json); }
      catch (e) { json(res, 400, { error: e.message }); return; }
    }
  ...
  if (u.pathname === '/zodiac') return routeZodiac(u, res, json);
  ...
  if (u.pathname === '/callingcode') return routeCallingcode(u, res, json);
  ...
  if (u.pathname === '/ipinfo') {
    ...
    if (u.pathname === '/email') {
      try { return routeEmail(u, res, json); }
      catch (e) { json(res, 400, { error: e.message }); return; }
    }
    if (u.pathname === '/url') {
