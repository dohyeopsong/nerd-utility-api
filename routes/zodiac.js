...
const WEST = [
  ['Capricorn', 120, 119], ['Aquarius', 120, 218], ['Pisces', 219, 320], ['Aries', 321, 419],
...
const CN_ELEM = ['Metal','Water','Wood','Fire','Earth'];

function westernSign(m, d) {
  const md = m * 100 + d;
  if (md >= 120 || md <= 119) return 'Capricorn';
  for (const [name, start, end] of WEST.slice(1)) {
    const s = Math.floor(start / 100) * 100 + (start % 100);
    const e = Math.floor(end / 100) * 100 + (end % 100);
    if (md >= s && md <= e) return name;
  }
  return 'Capricorn';
}
...
function routeZodiac(u, res, json) {
  ...
  const animal = CN[idx];
  const elem = CN_ELEM[Math.floor(((((y - 2020) % 10) + 10) % 10) / 2)];
  return json(res, 200, {
    date: dateStr,
    western_sign: sign,
